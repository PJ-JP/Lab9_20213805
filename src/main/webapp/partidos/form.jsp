<%@ page import="com.example.lab9_base.Bean.SeleccionNacional" %>
<%@ page import="com.example.lab9_base.Bean.Arbitro" %>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<jsp:useBean id="selecciones" scope="request" type="java.util.ArrayList<com.example.lab9_base.Bean.SeleccionNacional>"/>
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-6">
                    ...
                    <form method="POST" action="<%=request.getContextPath()%>/PartidoServlet?action=guardar">
                        ...
                        <div class="form-group">
                            <label for="selLocal">Selección local</label>
                            <select id="selLocal" name="local" class="form-control">
                                <%for (SeleccionNacional seleccionNacional: selecciones){%>
                                <option value="<%=seleccionNacional.getIdSeleccion()%>"><%=seleccionNacional.getNombre()%></option>
                                <%}%>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="selVisit">Selección Visitante</label>
                            <select id="selVisit" name="visitante" class="form-control">
                                <%for (SeleccionNacional seleccionNacional: selecciones){%>
                                <option value="<%=seleccionNacional.getIdSeleccion()%>"><%=seleccionNacional.getNombre()%></option>
                                <%}%>
                            </select>
